...
for(i in 1:length(marker.info.corrected)){
  ...
  write.table(ord, 
              paste0('March2023analysis/cluster', names(marker.info.corrected)[i], 'markers.txt'), 
              quote=F)
}
...
```
Repeating this using logcounts because the reconstructed data can have odd interpolated values.

```{r}
# note sce_all and sce_all_corrected have the same columns
...
library(pheatmap)
mat <- exprs(sce_all)[gene_list, sce_all_corrected$cluster == '11']
mat_doub <- sce_all_corrected$query_doublet[sce_all_corrected$cluster =='11']
rownames(mat) <- rowData(sce_all_corrected)[rownames(mat), 'Symbol']
colannot <- data.frame(doublet=factor(mat_doub))
rownames(colannot) <- colnames(mat)
...
mat <- exprs(sce_all)[gene_list, sce_all_corrected$cluster %in% c('1', '4')]
mat_clust <- sce_all_corrected$cluster[sce_all_corrected$cluster %in% c('1', '4')]
## downsample 
set.seed(100)
inds <- sample.int(length(mat_clust), 0.005*length(mat_clust), replace=FALSE)
...
mat2 <- exprs(sce_all)[gene_list, sce_all_corrected$cluster %in% c('2', '3')]
mat2_clust <- sce_all_corrected$cluster[sce_all_corrected$cluster %in% c('2', '3')]
## downsample 
set.seed(100)
inds <- sample.int(length(mat2_clust), 0.05*length(mat2_clust), replace=FALSE)
...
plotExpression(sce_all, features=fewgenes, x='cluster')
plotExpression(sce_all, features=fewgenes, x='cluster', exprs_values='counts')

```

Interestingly, the doublet annotation doesn't line up with the suspected doublets here. There are a handful of cells expressing a number of B cell markers and all of the cells express a pretty high level of JCHAIN. When we take into account the cluster-defining likely myeloid genes, the cells also exhibit robust expression of these (e.g. CD163). Thus, it is either a set of odd cells or doublets. It seems unlikely to be pure cluster contamination.
...
for(i in 1:length(marker.info.corrected)){
  if(names(marker.info.corrected)[i] %in% c('18a', '18b', '20a', '20b', '9a', '9b', '9c')){
    ord <- marker.info.corrected[[i]][order(marker.info.corrected[[i]]$mean.logFC.cohen, decreasing=TRUE),]
    ord$symbol <- rowData(sce_all)[rownames(ord), 'Symbol']
    write.table(ord, 
                paste0('March2023analysis/cluster', names(marker.info.corrected)[i], 'markers.txt'), 
...
pdf('March2023analysis/cluster_occupancy.pdf')
ggplot(dat, aes(fill=sample, y=n_cells, x=cluster)) +
  geom_bar(position='stack', stat='identity')
dev.off()
...
pdf('March2023analysis/UMAP_clusters.pdf')
plotUMAP(sce_all_corrected, colour_by='cluster', point_size=0.3)
dev.off()

png('March2023analysis/UMAP_clusters.png')
plotUMAP(sce_all_corrected, colour_by='cluster', point_size=0.3)
dev.off()
```
