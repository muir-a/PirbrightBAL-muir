...
pigPBMCsce$cell_type1[pigPBMCsce$cell_type1=='ASC'] <- 'Antibody secreting cell'
paste(unique(colData(pigPBMCsce)$cell_type1))
...
pigBALsce <- selectFeatures(pigBALsce, n_features=200, suppress_plot = FALSE)

pigPBMCsce <- selectFeatures(pigPBMCsce, n_features=200, suppress_plot = FALSE)
```
...
png(file='Data/Sankey_cmap_cell-types.png', height=1080, width=2160)
plot(getSankey(colData(pigBALsce)$cell_type1,  BAL_to_PBMC$scmap_cluster_labs[,1]))
dev.off()
```

Overall scmap hasn't worked, either the two datasets don't share genes in their respective cell types or scmap is failing somehow. An oddity is that the projection only seems to be mapping to 3-5 cell types (rather than all cell types), evidently something isn't functioning correctly. # Too many cells?#

Will try cell to cell mapping as well since everything is set up. 
...
png(file='Data/Sankey_cmap_cell-cell.png', height=1080, width=2160)
plot(
  getSankey(
...
dev.off()
```
scmap cell-cell mapping is still mapping the vast majority to unassigned, hence this is still not viable. 
...
```{r run mnnCorrect}
#mnnCorrected <- mnnCorrect(logcounts(pigBALsce), logcounts(pigPBMCsce), k=200)
...
dataset_labels <- factor(rep(c("m", "s"), times=c(ncol(pigBALsce), ncol(pigPBMCsce))))
cell_type_labels <- factor(c(colData(pigBALsce)$cell_type1, colData(pigPBMCsce)$cell_type1))
...
```{r save everything}
sessionInfo()
save(list=ls(), file='Cross_ref.RData') 
```