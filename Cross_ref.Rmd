...
head(BAL_genes[!(BAL_genes %in% PBMC_genes)], n=100)

head(PBMC_genes[!(PBMC_genes %in% BAL_genes)], n=100)
...
#top_HVGs_PBMC <- getTopHVGs(pigPBMC, n=2000)
...
for (n in 1:length(rownames(pigBALsce))){
...
# Redo gene annotation to see if this improves shared genes
...
colData(pigPBMCsce)$cell_type1 <- pigPBMCsce$celltypes
...
Calculate indexes for all shared genes and all cell types

```{r}
pigBALsce <- selectFeatures(pigBALsce, suppress_plot = FALSE)

pigPBMCsce <- selectFeatures(pigPBMCsce, suppress_plot = FALSE)
```
