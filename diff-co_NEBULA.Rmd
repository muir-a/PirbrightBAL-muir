...
GO_Genes <- GO_Genes[names(GO_Genes) %in% GO_Results$ID]
GO_Genes <- GO_Genes[match(GO_Results$ID, names(GO_Genes)),]

# obsolete function to show only significant genes
n <- 0
for(term in GO_Genes){
  n <- n + 1
  term <- term[term %in% sigGenes(GO_Data)]
  if(length(term) == 0){
    GO_Genes[n] <- "NA"
  } else {
  GO_Genes[n] <- term
