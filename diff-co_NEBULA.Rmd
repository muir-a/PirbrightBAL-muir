...
Create_Set <- function(Data){
  ...
  DE_subset <- Data[(Data$diffexpressed == "UP") | (Data$adj.P.Val < 0.001),]
...
options("warn"=-1)
venndir(PH1N1_Set, main="pH1N1", poly_alpha=0.5, proportional=TRUE, set_colors = set_colours, label_preset="main items", show_items = c("sign item"), font_cex = c(3,2,1), item_cex = c(2,2), overlap_type="each")
dev.off()

png(filename='Results/Venn_HANP.png', height=1080, width=2500)
...
options("warn"=-1)
venndir(HANP_Set, main="Ad-HA/NP", poly_alpha=0.5, proportional=TRUE, set_colors = c('#FF999A', '#A6CEE3'), label_preset="main items", show_items = c("sign item"), font_cex = c(3,2,1), item_cex = 4, overlap_type="each")
dev.off()
...
options("warn"=-1)
venndir(IL1b_Set, main="Ad-HA/NP Ad-IL1b", poly_alpha=0.5, proportional=TRUE, set_colors = set_colours, label_preset="main items", show_items = c("sign item"), font_cex = c(3,2,1), item_cex = 2.5, overlap_type="each")
dev.off()

Create_Set2 <- function(Data){
  #DE_subset <- Data[Data$diffexpressed != "NO",]
  DE_subset <- Data[Data$adj.P.Val < 0.05,]
  DE_subset <- DE_subset[(DE_subset$logFC > 0.6) | (DE_subset$logFC < -0.6),]
  DE_subset <- DE_subset[!is.na(DE_subset$ID),]
  DE_subset <- DE_subset[!grepl("ENSSSCG", DE_subset$ID, fixed = TRUE),]
  Set <- DE_subset$diffexpressed == "UP"
  Set[Set == TRUE] <- 1
...
png(filename='Results/Venn_IFI6.png', height=1080, width=2500)
...
options("warn"=-1)
venndir(IFI6_Set, poly_alpha=0.5, proportional=TRUE, set_colors = set_colours, label_preset="main items", show_items = c("sign item"), font_cex = c(3,2,1), item_cex = c(3,3), overlap_type="each")
dev.off()

```
old venn diagrams (unused)
```{r oldVenn, eval=FALSE}
library(vctrs)
library(venndir)
set_colours <- c('#E31A1C', '#B2DF8A', '#33A02C', '#A6CEE3')
theme_set(theme_minimal(base_size = 25))

# make function to create sets from NEBULA Data
Create_Set <- function(Data){
  #DE_subset <- Data[Data$diffexpressed != "NO",]
  DE_subset <- Data[Data$adj.P.Val < 0.01,]
  DE_subset <- DE_subset[!is.na(DE_subset$ID),]
  DE_subset <- DE_subset[!grepl("ENSSSCG", DE_subset$ID, fixed = TRUE),]
  Set <- DE_subset$diffexpressed == "UP"
  Set[Set == TRUE] <- 1
...
PH1N1_Set <- list_drop_empty(PH1N1_Set)
options("warn"=-1)
venndir(PH1N1_Set, main="pH1N1", poly_alpha=0.5, proportional=TRUE, set_colors = set_colours, label_preset="main items", show_items = c("sign item"), font_cex = c(3,2,1), item_cex = c(1,2), overlap_type="each")
dev.off()
...
HANP_Set <- list_drop_empty(HANP_Set)
options("warn"=-1)
venndir(HANP_Set, main="Ad-HA/NP", poly_alpha=0.5, proportional=TRUE, set_colors = c('#E31A1C', '#33A02C'), label_preset="main items", show_items = c("sign item"), font_cex = c(3,2,1), item_cex = 4, overlap_type="each")
dev.off()
...
IFI6_Set <- list_drop_empty(IFI6_Set)
options("warn"=-1)
venndir(IFI6_Set, poly_alpha=0.5, proportional=TRUE, set_colors = set_colours, label_preset="main items", show_items = c("sign item"), font_cex = c(2,2,1), item_cex = c(1.5,2), overlap_type="each")
dev.off()
```
...
for (Data in NEBULA_Data) {
  ...
  GO_Results <- GenTable(GO_Data, classicFisher = resultFisher, classicKS = resultKS, elimKS = resultKS.elim, orderBy = "classicFisher", ranksOf = "classicFisher", numChar=100, topNodes=20)
...
ggplot(GO_pH1N1, aes(x=dataset, y=-log(as.numeric(classicFisher)), size=Significant, label=Term, color=dataset)) +
  ...
  labs(color=NULL, size="count DE genes", title="pH1N1") + 
...
ggplot(GO_HANP, aes(x=dataset, y=-log(as.numeric(classicFisher)), size=Significant, label=Term, color=dataset)) +
  ...
  geom_hline(yintercept=-log10(0.05), col="red") +
  geom_jitter(width = 0.2, height = 0) +
  labs(color=NULL, size="count DE genes", title="Ad-HA/NP") + 
  ylab("-log(p-value)") + xlab(NULL) +
  scale_x_discrete(labels= c("All Cells", "Macrophages", "CD4 T Cells", "CD8 T Cells", "B Cells")) +
  scale_color_manual(values=five_colours) +
  guides(color="none") + 
...
ggplot(GO_IL1b, aes(x=dataset, y=-log(as.numeric(classicFisher)), size=Significant, label=Term, color=dataset)) +
  ...
  geom_hline(yintercept=-log10(0.05), col="red") +
  geom_jitter(width = 0.2, height = 0) +
  labs(color=NULL, size="count DE genes", title="Ad-HA/NP+IL1b") + 
  ylab("-log(p-value)") + xlab(NULL) +
  scale_x_discrete(labels= c("All Cells", "Macrophages", "CD4 T Cells", "CD8 T Cells", "B Cells")) +
  scale_color_manual(values=five_colours) +
  guides(color="none") + 
