...
n <- 0
ID_List <- c("HANP_BCells_genes", "HANP_CD4_genes", "HANP_CD8_genes", "HANP_Macrophages_genes", "IFI6_BCells_genes", "IFI6_CD4_genes", "IFI6_CD8_genes", "IFI6_Macrophages_genes", "IL1b_BCells_genes", "IL1b_CD4_genes", "IL1b_CD8_genes", "IL1b_Macrophages_genes", "PH1N1_BCells_genes", "PH1N1_CD4_genes", "PH1N1_CD8_genes", "PH1N1_Macrophages_genes")

for (genelist in c(HANP_BCells_genes, HANP_CD4_genes, HANP_CD8_genes, HANP_Macrophages_genes, IFI6_BCells_genes, IFI6_CD4_genes, IFI6_CD8_genes, IFI6_Macrophages_genes, IL1b_BCells_genes, IL1b_CD4_genes, IL1b_CD8_genes, IL1b_Macrophages_genes, PH1N1_BCells_genes, PH1N1_CD4_genes, PH1N1_CD8_genes, PH1N1_Macrophages_genes)){
  n <- n + 1
  ...
  GO_ont <- new("topGOdata", ontology = "BP", allGenes = gene_factor, annot = annFUN.org, mapping = "org.Ss.eg.db", ID="symbol")
  resultFisher <- runTest(test_GO, algorithm = "classic", statistic = "fisher")
  resultKS <- runTest(test_GO, algorithm = "classic", statistic = "ks")
  resultKS.elim <- runTest(test_GO, algorithm = "elim", statistic = "ks")
  GO_Results <- GenTable(test_GO, classicFisher = resultFisher, classicKS = resultKS, elimKS = resultKS.elim, orderBy = "elimKS", ranksOf = "classicFisher", topNodes = 10)
  ID <- ID_List[n]
  write.table(GO_Results, paste0('Results/GO_Results_', ID, '.txt'), sep='\t', quote=FALSE)
}
