...
for (n in 1:length(BAL_feature_genes)){
  top_genes <- rownames(BAL_feature_genes[[n]])[1:50]
...
for (n in 1:length(PBMC_feature_genes)){
  top_genes <- rownames(PBMC_feature_genes[[n]])[1:50]
...
#pigBALsce <- selectFeatures(pigBALsce, n_features=400, suppress_plot = FALSE)
...
#pigPBMCsce <- selectFeatures(pigPBMCsce, n_features=400, suppress_plot = FALSE)
...
PBMC_feature_genes <- rownames(metadata(pigPBMCsce)$scmap_cluster_index)

feature_BAL_sce <- pigBALsce[rowData(pigBALsce)$feature_symbol %in% PBMC_feature_genes, ]
feature_PBMC_sce <- pigPBMCsce[rowData(pigPBMCsce)$feature_symbol %in% PBMC_feature_genes, ]
...
#start_time <- Sys.time()
#fastmnnCorrected <- fastMNN(logcounts(pigBALsce), logcounts(pigPBMCsce), k=20)
#end_time <- Sys.time()
...
start_time <- Sys.time()
#mnnCorrected <- mnnCorrect(logcounts(pigBALsce), logcounts(pigPBMCsce), k=5)
end_time <- Sys.time()
slowmnn_time <- end_time-start_time
slowmnn_time
save(mnnCorrected, file='Data/mnnCorrected_k5.RData')
