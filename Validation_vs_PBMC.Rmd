...
**Note: This script is compute heavy so it is strongly recommended to run on a compute-cluster/workstation with more than 32 GB of memory and at least 16cores.**
...
PBMC_colour_list[grep('cDCs', pigPBMCsce$EXP_celltype)] <- '#FFFF00' #yellow
PBMC_colour_list[grep('pDCs', pigPBMCsce$EXP_celltype)] <- '#FFFF00' #yellow
...
PBMC_colour_list[grep('NK cells', pigPBMCsce$EXP_celltype)] <- '#6A3D9A' #purple
...
save(mnnCorrected, file='Data/mnnCorrected_feature50_k20.RData')
#load('Data/mnnCorrected_feature50_k20.RData')
mnnCorrected
```
...
plot1 <- plotUMAP(mnnCorrected, point_size=0) +
  ...
  geom_text(x=-12, y=16, size=30, label="A", color="black") +
...
plot2 <- plotUMAP(mnnCorrected, colour_by="batch", point_size=0.3) +
  ...
  geom_text(x=-12, y=16, size=30, label="B") +
...
plot3 <- plotUMAP(mnnCorrected_T_Cells, colour_by="EXP_celltype", point_size=0.3) +
  ...
  geom_text(x=-8.5, y=10, size=30, label="C") +
...
grouplist[grep('pDCs', nodes$name)] <- 'a'
...
grouplist2[grep('pDCs', links$source)] <- 'a'
...
my_color <- 'd3.scaleOrdinal(d3.schemeCategory10) .domain(["a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k", "l"]) .range(["#1F78B4", "#A6CEE3", "#B2DF8A", "#CAB2D6", "#33A02C", "#6A3D9A", "#FB9A99", "#E31A1C", "#FFFF00", "#FDBF6F", "#FF7F00", "#B15928"])'
...
heatmap.2(heat_all_Matrix, margins=c(30,50), scale="row", col = bluered(100), cexRow=2, cexCol=2, colCol=xColours,  trace="none", key=TRUE, lhei=c(1,10), lwid=c(1,10), ColSideColors=c(Col_Colours))
dev.off()
...
png(filename='Results/B_Cells_Cross_Heatmap.png', height=2160, width=2160)
heatmap.2(B_Cells_Matrix, Colv=FALSE, margins=c(50,40), scale="none", trace="none", dendrogram="row", col=viridis(256), lhei=c(0.1,10), lwid=c(1,10), cexRow=4, cexCol=4, key=FALSE)
dev.off()

png(filename='Results/Cross_Heatmap_with_scale.png', height=2160, width=2160)
heatmap.2(B_Cells_Matrix, Colv=FALSE, margins=c(15,15), scale="none", trace="none", dendrogram="row", col=viridis(256), lhei=c(1,9), key=TRUE, keysize=2, density.info='none', key.xlab="mean logcounts")
dev.off()

# CD8 and NK
CD8NK_genes <- c("CD28", "CD5", "CD3D", "CD3E", "CD4", "CD8A", "CD8B", "FOXP3", "E2F1", "E2F2", "KLRK1", "KLRB1", "KIT", "NCR1", "TBX21", "EOMES", "GATA3", "IL1RL1", "IL7R", "ID3", "PLVAP", "PRF1", "GPR37", "ENSSSCG00000026217", "ZNF683", "ENSSSCG00000008898", "CD5", "EVL", "CD247", "RHEX", "TRAT1", "NKG7", "GZMB", "CD2", "ENSSSCG00000009327", "SAT1", "LAT", "SH2D1A", "DAPL1")

BAL_CD8NK <- corrected_BAL[rownames(corrected_BAL) %in% CD8NK_genes, ]
BAL_CD8NK <- BAL_CD8NK[, BAL_CD8NK$cluster %in% c("9a", "9b", "9c", "21")]
PBMC_CD8NK <- corrected_PBMC[rownames(corrected_PBMC) %in% CD8NK_genes, ]
...
png(filename='Results/CD8NK_Cross_Heatmap.png', height=2160, width=2160)
heatmap.2(CD8NK_Matrix, margins=c(50,40), scale="none", trace="none", dendrogram="row", col=viridis(256), lhei=c(0.1,10), lwid=c(1,10), cexRow=4, cexCol=4, key=FALSE)
dev.off()
...
png(filename='Results/mitotic_T_cells_Cross_Heatmap.png', height=2160, width=2160)
heatmap.2(mitotic_matrix, margins=c(50,40), scale="none", trace="none", dendrogram="row", col=viridis(256), lhei=c(0.1,10), lwid=c(1,10), cexRow=4, cexCol=4, key=FALSE)
dev.off()
...
png(filename='Results/conventional_T_cells_Cross_Heatmap.png', height=2160, width=2160)
heatmap.2(T_Cell_Matrix, margins=c(50,40), scale="none", trace="none", dendrogram="row", col=viridis(256), lhei=c(0.1,10), lwid=c(1,10), cexRow=4, cexCol=4, key=FALSE)
dev.off()
