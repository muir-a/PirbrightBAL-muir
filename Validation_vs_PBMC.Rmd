...
Use original gene counts (of selected features), subset by interesting comparisons (cell-types), sort, and plot as heatmaps. Also calculate gene correlation coefficients to rank mapping genes. 
...
# Create gene ranking table by correlation coefficient

#gtable <- apply(x=c(BAL_B_cells, PBMC_B_cells, 1, cor)) #by row?
#cor(x, y, method="pearson") # x=BAL_B_cells$gene, y=PBMC_B_cell$gene
...
Run mnnCorrect
**Warning: this step is compute heavy. Will take several days/hours to complete.**
```{r run-mnnCorrect}
