...
Process for easy annotation, normalisation and batch correction. 
```{r gene-lists}
dim(sce_all_filt)
...
rownames(sce_all_filt) <- sce_all_filt$Symbol
...
genes  <- get_condition_genes(1000)
Tregs_dataSCE <- Tregs_dataSCE[rownames(dataSCE) %in% genes, ]
# Remove zero genes
...
genes  <- get_condition_genes(1000)
sub_dataSCE <- sub_dataSCE[rownames(sub_dataSCE) %in% genes, ]
...
sum(names(sub_g1["IFI6"]) %in% names(sub_g2["IFI6"]))/nrow(names(sub_g1["IFI6"]))
sum(names(sub_g1["IFI6"]) %in% names(sub_g3["IFI6"]))/nrow(names(sub_g1["IFI6"]))
sum(names(sub_g2["IFI6"]) %in% names(sub_g3["IFI6"]))/nrow(names(sub_g2["IFI6"]))
```
