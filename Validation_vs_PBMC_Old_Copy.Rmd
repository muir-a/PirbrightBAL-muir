...
PBMC_colour_list[grep('cDCs', pigPBMCsce$EXP_celltype)] <- '#FFFF99' #pale yellow
PBMC_colour_list[grep('pDCs', pigPBMCsce$EXP_celltype)] <- '#1F78B4' #dark blue
...
map_dataframe <- map_dataframe[map_dataframe$Freq > 0.16, ]
...
my_color <- 'd3.scaleOrdinal(d3.schemeCategory10) .domain(["a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k", "l"]) .range(["#1F78B4", "#A6CEE3", "#B2DF8A", "#CAB2D6", "#33A02C", "#6A3D9A", "#FB9A99", "#E31A1C", "#FFFF99", "#FDBF6F", "#FF7F00", "#B15928"])'
...
colour_df <- rbind(unique(as.data.frame(colData(pigBALsce))[, c("verbose_cluster", "colour")]), unique(as.data.frame(colData(pigPBMCsce))[, c("verbose_cluster", "colour")]))
...
save(fastmnnCorrected, file='Data/fastmnnCorrected_feature50_k20.RData')
fastmnnCorrected
...
Testing scmap on batch corrected data
...
# Trim flow, remove low values for legibility
corrected_map_dataframe <- corrected_map_dataframe[corrected_map_dataframe$Freq > 0.16, ]
